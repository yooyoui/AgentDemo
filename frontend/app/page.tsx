    ...
    <section className="main-area">
      ...
      {view === "workspace" && <div className="workspace">
        <section className="control-panel">
          ...
          <div className="customer-select-row"><select value={selectedId} onChange={(event) => setSelectedId(event.target.value)}><option value="">请选择</option>{customers.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select><button type="button" className="danger-icon" disabled={!selected || busy} onClick={() => { setDeleteName(""); setDeleteOpen(true); }} title="删除当前客户"><Trash2 size={17} /></button></div>
          {selected && <div className={`identity-status ${identity?.verification_status || "missing"}`}><div><span>{identity?.verification_status === "verified" ? "主体已核实" : identity?.verification_status === "unverified" ? "主体未核实（已明确沿用输入）" : "尚未确认组织主体"}</span>{identity && <small>输入：{identity.entered_name} · 标准名称：{identity.canonical_name}</small>}</div><button type="button" className="identity-button" disabled={busy || resolutionBusy} onClick={() => void startResolution("selected")}><Search size={14} />{identity ? "重新识别" : "识别主体"}</button></div>}
